...
export function LocationInput({
  ...
  // Fetch OSM suggestions when typing
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        const res = await fetch(
          ...
          {
            headers: {
              "User-Agent": "MyNextApp/1.0 (email@example.com)",
            },
          }
        );
  ...
  const handleSelectSuggestion = (sug) => {
    ...
    if (onCoordinatesChange) {
      onCoordinatesChange(parseFloat(sug.lat), parseFloat(sug.lon));
    }
    setSuggestions([]);
  };
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        {suggestions.length > 0 && (
          <ul className="absolute z-10 bg-zinc-800 w-full mt-1 rounded-lg shadow-lg max-h-60 overflow-auto">
            {suggestions.map((sug) => (
              <li
                key={sug.place_id}
